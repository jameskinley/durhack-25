...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!journeyId || !Array.isArray(points) || points.length < 2 || !Number.isFinite(Number(duration))) {
      return new Response(JSON.stringify({ error: 'invalid request payload' }), { status: 400, headers: corsHeaders(req) });
    }
    ...
    return new Response(JSON.stringify({ error: 'failed to curate' }), { status: 500, headers: corsHeaders(req) });
  }
});
